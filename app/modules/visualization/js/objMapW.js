...
policycompass.viz.mapW = function (options) {
    ...
    var λ = d3.scale.linear()//.domain([0, width])
        .domain([self.margin.left, self.margin.left + self.width]).range([-180, 180]);

    var φ = d3.scale.linear()//.domain([0, height])
        .domain([self.margin.top, self.height]).range([90, -90]);
    ...
    function setup(width, height) {
        if (self.projection == 'mercator') {
            projection = d3.geo.mercator().translate([
                (width / 2), (height / 2)
            ]).scale(width / 2 / Math.PI).precision(.1);
            ;
        } else if (self.projection == 'conicConformal') {
            projection = d3.geo.conicConformal()//.rotate([98, 0])
                .center([0, 0])//.parallels([29.5, 45.5])
                //.scale(1000)
                .scale(width / 2 / Math.PI)//.scale( width / 4 / Math.PI)
                //.translate([width / 2, height / 2])
                .precision(.1);
        } else if (self.projection == 'equirectangular') {

            projection = d3.geo.equirectangular().translate([(width / 2), (height / 2)])//.scale( width / 2 / Math.PI)
                .precision(.1);
            ;
            ...
            projection = d3.geo.orthographic()// .scale(475)
                .translate([width / 2, height / 2]).clipAngle(90).precision(.1);
        } else if (self.projection == 'azimuthalEqualArea') {
            projection = d3.geo.azimuthalEqualArea().clipAngle(180 - 1e-3)//.scale(237)
        ...
        self.svg = d3.select(self.parentSelect).append("svg").attr("width", width).attr("height", height).attr("class", "mapa")//.call(zoom)
            //.on("click", click)
            .append("g")
        ...
        if (self.legend) {
            ...
            for (var i = 0; i < 25; i++) {
                ...
                self.svg.append("text").attr("x", function (d, i) {
                    //console.log("cnti="+cnti+"--key="+key);
                    return valueX;
                }).attr("y", function (d, i) {
                    //console.log("--->cnti="+cnti+"--key="+key);
                    return valueY;
                }).attr("text-anchor", "center").attr("text-decoration", "none").attr("class", "link superior legend value").attr("font-size", self.font_size).text("A" + i).style("fill", function (d, i) {
                    return color(key);
                });
                ;
            }
...
//	d3.select(self.frameElement).style("height", height + "px");
    ...
    function draw(topo) {
        self.svg.append("path").datum(graticule).attr("class", "graticule").attr("fill", "none").attr("d", path).on("mousemove", function (d, i) {
        ...
        country.on("mousemove", function (d, i) {
            ...
            var mouse = d3.mouse(self.svg.node()).map(function (d) {
                return parseInt(d);
            });
    ...
    function addpoint(lat, lon, text) {
        ...
        gpoint.append("svg:circle").attr("cx", x).attr("cy", y).attr("class", "point").attr("r", 2).style("fill", "blue");

        //conditional in case a point has no associated text
        if (text.length > 0) {
            gpoint.append("text").attr("x", x + 5).attr("y", y + 2).attr("class", "text").style("fill", "blue").text(text);
        }
