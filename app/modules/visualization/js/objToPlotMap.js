...
policycompass.viz.mapW_datamaps = function (options) {
    ...
    var zoomFactor = 0.9, enabled = true;
    ...
    self.plotMap = function () {
        ...
        d3.json("/app/modules/visualization/json/world-topo-min.json", function (error, world) {
            ...
            for (var i = 0; i < countries.length; i++) {
                for (di in self.data) {
                    ...
                    if (sum > 0) {
                        ...
                        if (self.showBubbles) {
                            arrayBubbles.push({
                                'name': countries[i].properties.name + ': ' + sum,
                                'latitude': latlng[1],
                                'longitude': latlng[0],
                                'radius': radious,
                                'fillKey': sum
            ...
            self.svg = d3.select(self.parentSelect).append("svg").attr("width", width).attr("height", height).attr("class", "mapa").append("g");
            ...
            var map_to_plot = new Datamap({
                ...
                setProjection: function (element, options) {
                    ...
                    if (self.projection == 'mercator') {
                        projection = d3.geo.mercator().center([0, 40])//.center([lt, lg])
                            .translate([(width / 2), (height / 2)]).scale(width / 3.8 / Math.PI).precision(.1);

                    } else if (self.projection == 'conicConformal') {
                        projection = d3.geo.conicConformal()//.rotate([98, 0])
                            .center([0, 40])//.parallels([29.5, 45.5])
                            //.scale(1000)
                            .translate([
                                (width / 2), (height / 2)
                            ]).scale(width / 3 / Math.PI)//.scale( width / 4 / Math.PI)
                            //.translate([width / 2, height / 2])
                            .precision(.1);
                    } else if (self.projection == 'equirectangular') {
                        projection = d3.geo.equirectangular().translate([
                            (width / 2), (height / 2)
                        ]).scale((width) / 3 / Math.PI)//.scale( width / 2 / Math.PI)
                            .precision(.1);
                        ;
                        ...
                        projection = d3.geo.azimuthalEqualArea().clipAngle(180 - 1e-3)//.scale(237)
                            .scale((width) / 2 / Math.PI).translate([width / 2, height / 2]).precision(.1);
                    } else {
                ...
                height: height,
                width: width, //done: function() {
                //
                //},


                geographyConfig: {
                    highlightBorderColor: '#bada55', //popupOnHover: false,
                ...
                //},
                    ...
                    if (self.showZoom) {
                        ...
                        function redraw() {
                            ...
                            datamap.svg.selectAll("circle").attr("r", function () {
