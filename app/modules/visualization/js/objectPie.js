...
policycompass.viz.pie = function (options) {
    ...
    self.drawArcs = function (piesArray) {
        ...
        if (showLabels) {
            ...
            self.g.append("svg:text").attr("class", "text text_pie_" + self.idName)//.style("stroke", function(d,i) {return colorScale(i);})
                /*
                ...
                .attr("stroke", "rgb(0, 0, 0)").attr("stroke-width", "0px")
        ...
        if (showLegend) {
            ...
            if (piesArray['Units'].length > 0) {
                self.svg.append("text")//.attr("x", -self.radius/2)
                    //.attr("x", self.radius)
                    .attr("y", self.radius).attr("dy", ".35em").attr("text-anchor", "center").attr("class", "link superior legend value").attr("font-size", self.font_size).style("stroke", function (d, i) {
                        return 'black';
                    }).text(function (d, i) {
            ...
            var legend = self.svg.selectAll(".legend_pie_" + self.idName)//.data(months.slice().reverse())
                .data(pie(pies)).enter().append("svg:g")//.attr("class", "legend_pie_"+self.idName)
            ...
            legend.append("rect")//.attr("class", "rect_pie_"+self.idName)
            ...
            legend.append("text")//.attr("class", "text_pie_"+self.idName)
                .attr("x", self.radius + 5 + 5 + 5).attr("y", 9).attr("font-size", self.font_size)//.attr("dy", ".35em")
                .attr("dy", "-.30em")//.style("text-anchor", "end")
    ...
    self.init = function () {
        ...
        self.svg = d3.select(self.parentSelect).append("svg").attr("class", "pc_chart").attr("id", "graph_" + self.idName).attr("width", self.width + self.margin.right + self.margin.left).attr("height", self.height + self.margin.top + self.margin.bottom).on("mousemove", mousemove)//.on("mouseover", console.log("aaaaaaa"))
            //.style("float","left")
            .append("g").attr("transform", "translate(" + (self.width + self.margin.right + self.margin.left) / 2 + "," + (self.height + self.margin.top + self.margin.bottom) / 2 + ")");
    }
